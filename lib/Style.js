...
class Style {
     ...
     */
    constructor(styleSheet, id, xfNode, fontNode, fillNode, borderNode, fileType) {
    ...
    _getColor(node, name) {
        ...
        if (child.attributes.hasOwnProperty('tint')) color.tint = (Math.round(child.attributes.tint * 100) / 100).toString();
    ...
    _get_bold() {
        const prefix = (this._fileType === "cell") ? "x:" : "";
        return xmlq.hasChild(this._fontNode, `${prefix}b`);
    }
    ...
    _get_italic() {
        const prefix = (this._fileType === "cell") ? "x:" : "";
        return xmlq.hasChild(this._fontNode,  `${prefix}i`);
    }
    ...
    _get_fontColor() {
        const prefix = (this._fileType === "cell") ? "x:" : "";
        return this._getColor(this._fontNode, `${prefix}color`);
    }
    ...
    _get_horizontalAlignment() {
        let key = "alignment";
        if (this._fileType === "cell") {
            key = `x:${key}`;
        }
        return xmlq.getChildAttribute(this._xfNode, key, "horizontal");
    }
    ...
    _get_verticalAlignment() {
        ...
        return xmlq.getChildAttribute(this._xfNode, key, "vertical");
    }
    ...
    _get_fill() {
        const prefix = (this._fileType === "cell") ? "x:" : "";
        const patternFillNode = xmlq.findChild(this._fillNode, `${prefix}patternFill`);
        const gradientFillNode = xmlq.findChild(this._fillNode, `${prefix}gradientFill`);
        const patternType = patternFillNode && patternFillNode.attributes.patternType;

        if (patternType === "solid") {
            return {
                type: "solid",
                color: this._getColor(patternFillNode, `${prefix}fgColor`)
            };
        }
        ...
        if (patternType) {
            return {
                type: "pattern",
                pattern: patternType,
                foreground: this._getColor(patternFillNode, `${prefix}fgColor`),
                background: this._getColor(patternFillNode, `${prefix}bgColor`)
            };
        }
        ...
        if (gradientFillNode) {
            ...
            const fill = {
                ...
                stops: _.map(gradientFillNode.children, stop => ({
                    ...
                    color: this._getColor(stop, `${prefix}color`)
                }))
            };
    ...
    _getBorder() {
        const result = {};
        const prefix = (this._fileType === "cell") ? "x:" : "";
        [`${prefix}left`, `${prefix}right`, `${prefix}top`, `${prefix}bottom`, `${prefix}diagonal`].forEach(side => {
            ...
            const color = this._getColor(sideNode, `${prefix}color`);
