...
class Style {
    ...
    _get_fill() {
        const prefix = (this._fileType === "cell") ? "x:" : "";
        const patternFillNode = xmlq.findChild(this._fillNode, `${prefix}patternFill`);// jq.get(this._fillNode, "patternFill[0]");
        const gradientFillNode = xmlq.findChild(this._fillNode, `${prefix}gradientFill`);// jq.get(this._fillNode, "gradientFill[0]");
