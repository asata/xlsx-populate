...
class StyleSheet {
     ...
     */
    constructor(node, fileType) {
        this._fileType = fileType;
        this._init(node, fileType);
    ...
    createStyle(sourceId) {
        ...
        if (sourceId >= 0) {
            ...
            if (this._fileType === "xlsx") {
                sourceXfNode = this._cellXfsNode.children[sourceId];
            } else {
                let temp = xmlq.findChild(this._cellXfsNode.children[sourceId], "mc:Choice");
                if (temp) {
                    sourceXfNode = xmlq.findChild(temp, "x:xf");
                } else {
                   sourceXfNode = this._cellXfsNode.children[sourceId];
                }
            }
            ...
            if (sourceXfNode?.attributes?.applyFont) {
                ...
                if (fontNode?.name === "mc:AlternateContent") {
                    fontNode = xmlq.findChild(xmlq.findChild(fontNode, "mc:Choice"), "x:font");
                }
            }
        ...
        const prefix = (this._fileType === "xlsx") ? "" : "x:";
        if (!fontNode) fontNode = { name: `${prefix}font`, attributes: {}, children: [] };
        this._fontsNode.children.push(fontNode);

        if (!fillNode) fillNode = { name: `${prefix}fill`, attributes: {}, children: [] };
        ...
        borderNode.children = [
            xmlq.findChild(borderNode, `${prefix}left`) || { name: `${prefix}left`, attributes: {}, children: [] },
            xmlq.findChild(borderNode, `${prefix}right`) || { name: `${prefix}right`, attributes: {}, children: [] },
            xmlq.findChild(borderNode, `${prefix}top`) || { name: `${prefix}top`, attributes: {}, children: [] },
            xmlq.findChild(borderNode, `${prefix}bottom`) || { name: `${prefix}bottom`, attributes: {}, children: [] },
            xmlq.findChild(borderNode, `${prefix}diagonal`) || { name: `${prefix}diagonal`, attributes: {}, children: [] }
        ];
        ...
        return new Style(this, this._cellXfsNode.children.length - 1, xfNode, fontNode, fillNode, borderNode, this._fileType);
    }
    ...
    _init(node, fileType) {
        ...
        const prefix = fileType === "cell" ? "x:" : "";
        // Cache the refs to the collections.
        this._numFmtsNode = xmlq.findChild(this._node, `${prefix}numFmts`);
        this._fontsNode = xmlq.findChild(this._node, `${prefix}fonts`);
        this._fillsNode = xmlq.findChild(this._node, `${prefix}fills`);
        this._bordersNode = xmlq.findChild(this._node, `${prefix}borders`);
        this._cellXfsNode = xmlq.findChild(this._node, `${prefix}cellXfs`);
        ...
        if (this._numFmtsNode) delete this._numFmtsNode.attributes.count;
        if (this._fontsNode) delete this._fontsNode.attributes.count;
        if (this._fillsNode) delete this._fillsNode.attributes.count;
        if (this._bordersNode) delete this._bordersNode.attributes.count;
        if (this._cellXfsNode) delete this._cellXfsNode.attributes.count;
    }
}
