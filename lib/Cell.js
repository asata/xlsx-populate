...
class Cell {
    ...
    _parseNode(node) {
        ...
        const fileType = this.workbook()._fileType;
        ...
        const fNode = xmlq.findChild(node, `${prefix}f`);
        if (fNode) {
        ...
        if (type === "s") {
            ...
            const isNode = xmlq.findChild(node, `${prefix}is`);
            if (isNode.children[0].name === `${prefix}t`) {
            ...
            // Boolean value.
            this._value = xmlq.findChild(node, `${prefix}v`).children[0] === 1;
        } else if (type === "e") {
            // Error value.
            const error = xmlq.findChild(node, `${prefix}v`).children[0];
