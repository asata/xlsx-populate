...
class Sheet {
    ...
    _init(workbook, idNode, node, relationshipsNode) {
        ...
        const prefix = (workbook._fileType === "cell") ? "x:" : "";
        this._sheetDataNode = xmlq.findChild(this._node, `${prefix}sheetData`);
        ...
        this._columns = [];
        this._colsNode = xmlq.findChild(this._node, `${prefix}cols`);
        if (this._colsNode) {
            xmlq.removeChild(this._node, this._colsNode);
        } else {
            this._colsNode = { name: `${prefix}cols`, attributes: {}, children: [] };
        }
        ...
        this._sheetPrNode = xmlq.findChild(this._node, `${prefix}sheetPr`);
        if (!this._sheetPrNode) {
            this._sheetPrNode = { name: `${prefix}sheetPr`, attributes: {}, children: [] };
            xmlq.insertInOrder(this._node, this._sheetPrNode, nodeOrder);
        }

        // Create the merge cells.
        this._mergeCellsNode = xmlq.findChild(this._node, `${prefix}mergeCells`);
        if (this._mergeCellsNode) {
            xmlq.removeChild(this._node, this._mergeCellsNode);
        } else {
            this._mergeCellsNode = { name: `${prefix}mergeCells`, attributes: {}, children: [] };
        }
        ...
        this._dataValidationsNode = xmlq.findChild(this._node, `${prefix}dataValidations`);
        if (this._dataValidationsNode) {
            xmlq.removeChild(this._node, this._dataValidationsNode);
        } else {
            this._dataValidationsNode = { name: `${prefix}dataValidations`, attributes: {}, children: [] };
        }
        ...
        for (let i = 0; i < this._node.children.length; i++) {
            if (this._node.children[i]["name"] === `${prefix}conditionalFormatting`) {
                this._conditionalFormatting.push(this._node.children[i]);
            }
        }
        // this._conditionalFormatting = xmlq.findChild(this._node, "conditionalFormatting");
        this._sortState = xmlq.findChild(this._node, `${prefix}sortState`);
        this._scenarios = xmlq.findChild(this._node, `${prefix}scenarios`);
        // "sheetPr", "dimension", "sheetViews", "sheetFormatPr", "cols", "sheetData",
        // "sheetCalcPr", "sheetProtection", "autoFilter", "protectedRanges", "scenarios", "autoFilter",
        // "sortState", "dataConsolidate", "customSheetViews", "mergeCells", "phoneticPr",
        // "conditionalFormatting", "dataValidations", "hyperlinks", "printOptions",
        ...
        // "drawingHF", "legacyDrawing", "legacyDrawingHF", "picture", "oleObjects", "controls", "webPublishItems", "tableParts",
        // "extLst"
        // console.log(JSON.stringify(xmlq.findChild(this._node, "scenarios")))
        // if (this._node?.children) {
        //     for (let c = 0; c < this._node.children.length; c++) {
        //         if (this._node.children[c]["name"] == "sortState") {
        //             console.log(this._node.children[c]["attributes"], JSON.stringifytthis._node.children[c]["children"])
        //         }
        //     }
        // }
        ...
        this._hyperlinksNode = xmlq.findChild(this._node, `${prefix}hyperlinks`);
        if (this._hyperlinksNode) {
            xmlq.removeChild(this._node, this._hyperlinksNode);
        } else {
            this._hyperlinksNode = { name: `${prefix}hyperlinks`, attributes: {}, children: [] };
        }
        ...
        this._printOptionsNode = xmlq.findChild(this._node, `${prefix}printOptions`);
        if (this._printOptionsNode) {
            xmlq.removeChild(this._node, this._printOptionsNode);
        } else {
            this._printOptionsNode = { name: `${prefix}printOptions`, attributes: {}, children: [] };
        }
        ...
        this._pageMarginsNode = xmlq.findChild(this._node, `${prefix}pageMargins`);
        if (this._pageMarginsNode) {
            ...
            this._pageMarginsPresetName = undefined;
            this._pageMarginsNode = { name: `${prefix}pageMargins`, attributes: {}, children: [] };
        }
