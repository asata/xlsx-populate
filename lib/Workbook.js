...
class Workbook {
    ...
    _initAsync(data, opts) {
        ...
        return externals.Promise.resolve()
            ...
            .then(nodes => {
                ...
                this._styleSheet = new StyleSheet(styleSheetNode, this._fileType);
                ...
                // Load each sheet.
                this._sheetsNode = xmlq.findChild(this._node, `${fileMode === "cell" ? "x:" : ""}sheets`);
                ...
                return externals.Promise.all(_.map(this._sheetsNode.children, (sheetIdNode, i) => {
                    ...
                    return this._parseNodesAsync([`xl/worksheets/sheet${i + 1}.xml`, `xl/worksheets/_rels/sheet${i + 1}.xml.rels`])
                        .then(nodes => {
                            ...
                            let rId = undefined;
                            for (let r = 0; r < relationshipsNode.children.length; r++) {
                                if (relationshipsNode.children[r]["attributes"]["Target"] === `worksheets/sheet${i + 1}.xml`) {
                                    rId = relationshipsNode.children[r]["attributes"]["Id"];
                                    break;
                                }
                            }
                            for (let s = 0; s < this._sheetsNode.children.length; s++) {
                                if (this._sheetsNode.children[s].attributes["r:id"] === rId) {
                                    sheetId = this._sheetsNode.children[s];
                                    break;
                                }
                            }

                            // Insert at position i as the promises will resolve at different times.
                            this._sheets[i] = new Sheet(this, sheetId, sheetNode, sheetRelationshipsNode);
                        });
                }));
            })
            .then(() => {
                ...
                for (let key in this._zip.files) {
                    // console.log(key)
                    ...
                    if (key.includes("styles.xml")) {
                        style.push(key);
                    }
                    if (key.includes("theme") && key.endsWith(".xml")) {
                        theme.push(key);
                    }
                }

                externals.Promise.all(_.map(pivot, name => this._zip.file(name)))
                .then(files => externals.Promise.all(_.map(files, file => file && file.async("string"))))
                .then(texts => externals.Promise.all(_.map(texts, text => {
                //     console.log(text);
                })))

                return externals.Promise.all(_.map([drawing, chart, pivot, style, theme, rels], (values, i) => {
                    return this._parseNodesAsync(values)
                    .then(nodes => {
                        if (this.drawing === undefined) this.drawing = [];
                        if (this.chart === undefined) this.chart = [];
                        if (this.pivot === undefined) this.pivot = [];
                        if (this.style === undefined) this.style = [];
                        if (this.theme === undefined) this.theme = {};
                        if (this.rels === undefined) this.rels = [];

                        for (let v = 0; v < values.length; v++) {
                            const temp = values[v].replace(".rels", "").split("/");
                            const keyName = temp[temp.length - 1];
                            if (i == 0) {
                                this.drawing.push({
                                    key: keyName,
                                    value: nodes[v]
                                ...
                                for (let t = 0; t < nodes[v]["children"][0]["children"].length; t++) {
                                    if (nodes[v]["children"][0]["children"][t]["name"] !== "a:fmtScheme") continue;
                                    for (let d = 0; d < nodes[v]["children"][0]["children"][t]["children"].length; d++) {
                                        let themeInfo = nodes[v]["children"][0]["children"][t]["children"][d];
                                        let themeType = "";
                                        if (themeInfo.name.includes("bgFill")) {
                                            if (!this.theme.bgFill) this.theme.bgFill = [];
                                            themeType = "bgFill";
                                        } else if (themeInfo.name.includes("fill")) {
                                            if (!this.theme.fill) this.theme.fill = [{}];
                                            themeType = "fill";
                                        } else if (themeInfo.name.includes("ln")) {
                                            if (!this.theme.ln) this.theme.ln = [{ noFill: true, }];
                                            themeType = "ln";
                                        } else if (themeInfo.name.includes("effect")) {
                                            if (!this.theme.effect) this.theme.effect = [{}];
                                            themeType = "effect";
                                        }

                                        for (let c = 0; c < themeInfo.children.length; c++) {
                                            if (themeType === "fill") {
                                                if (themeInfo.children[c]["name"] === "a:solidFill") {
                                                    this.theme.fill.push({ type:"solid" });
                                                } else if (themeInfo.children[c]["name"] === "a:gradFill") {
                                                    let item = this.getGradFill(themeInfo.children[c]);
                                                    item.type = "grad";
                                                    this.theme.fill.push(item);
                                                }
                                            } else if (themeType === "ln") {
                                                this.theme.ln.push(this.getLine(themeInfo.children[c]));
                                            } else if (themeType === "effect") {
                                                this.theme.effect.push(this.getEffect(themeInfo.children[c]));
                                            }
                                        }
                                    }
                                }
                            } else if (i == 5) {
                                for (let c = 0; c < nodes[v]["children"].length; c++) {
                                    const info = nodes[v]["children"][c]["attributes"];
                                    let obj = {
                                        fileName: keyName,
                                        id: info.Id,
                                        type: info.Type,
                                    ...
                                    if (info.Target.includes("jpeg")) {
                                        obj.fileData = this._zip.file(info.Target.replace("..", "xl"))._data.compressedContent;
                                    }
                                    this.rels.push(obj);
                                }
                            }
    ...
    getGradFill(root) {
        ...
        if (root) {
            ...
            for (let i = 0; i < root.children.length; i++) {
                if (root.children[i]["name"] === "a:gsLst") {
                    if (!gradFill.list) gradFill.list = [];
                    for (let j = 0; j < root.children[i].children.length; j++) {
                        let grad = root.children[i].children[j]["attributes"];
                        for (let d = 0; d < root.children[i]["children"][j]["children"][0]["children"].length; d++) {
                            const name = root.children[i]["children"][j]["children"][0]["children"][d]["name"].replaceAll("a:", "");
                            const value = root.children[i]["children"][j]["children"][0]["children"][d]["attributes"]["val"];
                            grad[name] = value;
                        }
                        gradFill.list.push(grad);
                    }

                } else if (root.children[i]["name"] === "a:path") {
                    if (!gradFill.path) gradFill.path = [];

                } else if (root.children[i]["name"] === "a:lin") {
                    gradFill.lin = root.children[i]["attributes"];
                }
                // const gsList = root["children"][i]["a:gsLst"];
                // if (gsList[0]["a:schemeClr"] || gsList[0]["a:srgbClr"]) {
                //     for (let j = 0; j < gsList.length; j++) {
                //         let grad = getSchemeClr(gsList[j]["a:schemeClr"] ? gsList[j]["a:schemeClr"] : gsList[j]["a:srgbClr"]);
                //         grad.pos = Number(gsList[j]["$"]["pos"]);
                //         if (!type) {
                //             if (!isPowerPoint && grad.alpha && grad.alpha === 100000) delete grad.alpha;
                //         }
                //         gradFill.list.push(grad);
                //     }
                // }
                ...
                // if (root[0]["a:path"]) {
                //     if (!gradFill.path) gradFill.path = [];
                //     for (let j = 0; j < root[0]["a:path"].length; j++) {
                //         let path = root[0]["a:path"][j]["a:fillToRect"][0]["$"];
                //         path.path = root[0]["a:path"][j]["$"]["path"];
                //         gradFill.path.push(path);
                //     }
                // }
            }
        }
    ...
    getLine(root) {
        let ln = root.attributes;
        for (let i = 0; i < root.children.length; i++) {
            if (root.children[i].name === "a:solidFill") { 
                for (let j = 0; j < root.children[i].children[0].children.length; j++) {
                    if (root.children[i].children[0].children[j].name === "a:shade") {
                        ln.shade = root.children[i].children[0].children[j].attributes.val;
                    } else if (root.children[i].children[0].children[j].name === "a:satMod") {
                        ln.satMod = root.children[i].children[0].children[j].attributes.val;
                    }
                }
            } else if (root.children[i].name === "a:prstDash") {
                ln.prstDash = root.children[i].attributes.val;
            } else if (root.children[i].name === "a:miter") {
                ln.miter = root.children[i].attributes.lim;
            }
        }
    ...
    getEffect(root) {
        let effect = {};
        for (let i = 0; i < root.children.length; i++) {
            if (root.children[i].name === "a:effectLst") { 
                for (let j = 0; j < root.children[i].children.length; j++) {
                    let name = root.children[i].children[j].name.replaceAll("a:","");
                    let value = root.children[i].children[j].attributes;
                    effect[name] = value;
                    if (root.children[i].children[j].children.length > 0) {
                        let colorName = root.children[i].children[j].children[0].name.replaceAll("a:","");
                        let colorValue = root.children[i].children[j].children[0].attributes;
                        effect[name][colorName] = colorValue;
                        for (let c = 0; c < root.children[i].children[j].children[0].children.length; c++) {
                            let detailName = root.children[i].children[j].children[0].children[c].name.replaceAll("a:","");
                            let detailValue = root.children[i].children[j].children[0].children[c].attributes.val;
                            effect[name][colorName][detailName] = detailValue;
                        }
                    }
                }
            } else if (root.children[i].name === "a:scene3d") { 
                for (let j = 0; j < root.children[i].children.length; j++) {
                    let name = root.children[i].children[j].name.replaceAll("a:","");
                    let value = root.children[i].children[j].attributes;
                    effect[name] = value;
                    if (root.children[i].children[j].children.length > 0) {
                        let detailName = root.children[i].children[j].children[0].name.replaceAll("a:","");
                        let detailValue = root.children[i].children[j].children[0].attributes;
                        effect[name][detailName] = detailValue;
                    }
                }
            } else if (root.children[i].name === "a:sp3d") { 
                for (let j = 0; j < root.children[i].children.length; j++) {
                    let name = root.children[i].children[j].name.replaceAll("a:","");
                    let value = root.children[i].children[j].attributes;
                    effect[name] = value;
